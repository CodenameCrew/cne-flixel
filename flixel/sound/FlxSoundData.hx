...
class FlxSoundData implements IFlxDestroyable
{
	...
	inline function get_bitsPerSample():Int
	{
		...
		if (buffer != null && buffer.bitsPerSample > 0) return buffer.bitsPerSample;
		return 16;
		#else
		return buffer != null ? buffer.bitsPerSample : 0;
		#end
	}
	...
	{
		#if (js && html5 && howlerjs)
		if (buffer != null && buffer.channels > 0) return buffer.channels;
		return 2;
		#else
	...
	{
		#if (js && html5 && howlerjs)
		if (buffer != null && buffer.sampleRate > 0) return buffer.sampleRate;
		...
		return 44100;
		#else
		return buffer != null ? buffer.sampleRate : 0;
		#end
	}
	...
	{
		#if (js && html5 && howlerjs)
		return (buffer != null && buffer.__srcHowl != null) ? Int64.fromFloat(buffer.__srcHowl.duration() * get_sampleRate()) : 0;
		#else
		if (buffer != null)
		{
			if (buffer.decoder != null) return buffer.decoder.total();
			else if (buffer.data != null) return Int64.make(0, Std.int(buffer.data.length / (buffer.bitsPerSample >> 3) / buffer.channels));
		}
		#end
		return 0;
	}
	...
	{
		#if (js && html5 && howlerjs)
		return (buffer?.__srcHowl?.duration() ?? 0) * 1000.0;
		#else
		if (buffer != null)
		{
			if (buffer.decoder != null)
			{
				var samples:Int64 = buffer.decoder.total();
				return (samples.high * 4294967296.0 + (samples.low >>> 0)) / buffer.sampleRate * 1000.0;
			}
			else
			{
				return buffer.data.length / (buffer.bitsPerSample >> 3) / buffer.channels / buffer.sampleRate * 1000.0;
			}
		}
		return 0;
		#end
	...
	{
		return buffer != null && buffer.data == null && buffer.decoder != null;
	}
